...
# system_prompt = """
# You are an expert assistant specializing in neuroscience and neural pathways. 
# Answer the user's question based on the following context and the chat history.Be concise and clear,do not output thinking stage and redudant results.
# After completing the text answer, you will also need to return the data in the original database that I used in the following format. Note that do not give repeated results and The Json format is like as, stright forwrd as the format, do not strat with other word:
...
system_prompt = """
### ROLE ###
You are a highly specialized AI assistant with expert knowledge in neuroscience and neural pathways.

### PRIMARY TASK ###
Your primary task is to answer the user's question accurately and concisely.

### INSTRUCTIONS ###
1.  **Analyze the User's Question**: Carefully understand the user's query provided in the '{{input}}'.
2.  **Consult Sources**: Base your answer STRICTLY on the information available in the provided '{{context}}'(database query results). DO NOT use any external knowledge or make assumptions.
3.  **Formulate Answer**: Generate a clear, concise, and professional textual answer to the user's question.
4.  **Identify Data Sources**: Identify the specific rows from the '{{context}}' that you used to formulate your answer.
5.  **Format Output**: Your final output MUST follow the two-part structure defined in the "OUTPUT FORMAT" section below.

### OUTPUT FORMAT ###
Your response MUST consist of two parts and nothing else:

**Part 1: Textual Answer**
- A direct answer to the user's question.

**Part 2: JSON Data Source**
- Start this part on a new line, immediately after the textual answer.
- Provide a JSON object containing ONLY the unique data rows from the '{{context}}' that were directly used to generate your answer.
- The JSON object must be the absolute last thing in your response. DO NOT add any introductory text like "Original Database:".
- The JSON object MUST strictly adhere to the following structure:
  ...
  "rows": [
    // Array of unique row objects used for the answer, this following is just one example, when you generate, you need to list all related data!
      [...,...,...], [...,...,...]
  ]

### CONSTRAINTS & EDGE CASES ###
- Your entire response MUST NOT contain any thought processes, self-corrections, or redundant text.
"""
...
def chat_with_rag(request: ChatRequest):
   ...
   print('answer',answer)
   import re
   # 更稳健的方式：分别匹配 head 和 rows，然后组合

   head_pattern = r'"head"\s*:\s*(\[[^]]*\])'
   head_match = re.search(head_pattern, answer, re.DOTALL | re.IGNORECASE)

   rows_pattern = r'"rows"\s*:\s*(\[(?:\s*\[[^[\]]*\][,\s]*)+\])'
   rows_match = re.search(rows_pattern, answer, re.DOTALL | re.IGNORECASE)

   head = json.loads(head_match.group(1))
   rows = json.loads(rows_match.group(1))

   if not head_match or not rows_match:
      raise ValueError('未找到 "head" 或 "rows" 字段')
   ...
   # answer 这个也切掉### Original Database:
   # answer = answer[:answer.find("Original Database:")] if "Original Database:" in answer else answer
   
   before_1, after_ans1 = answer.split("</think>\n\n", 1)
   before_2, after_2 = after_ans1.split("json", 1)


   
   # answer = answer[:min([i for i in [answer.find("json"), answer.find("### Original Database:")] if i != -1], default=len(answer))]

   return {"generated_text": before_2, "table_data":table_result}
